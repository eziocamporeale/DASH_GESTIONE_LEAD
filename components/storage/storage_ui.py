#!/usr/bin/env python3
"""
Interfaccia Utente per Storage Manager
...
def render_storage_wrapper():
    ...
    # Controlla sia is_admin che role_name per compatibilità
    is_admin = current_user and (
        current_user.get('is_admin', False) or 
        current_user.get('role_name') == 'Admin'
    )
    
    # Mostra informazioni utente
    if current_user:
        st.success(f"✅ Loggato come: {current_user.get('username')} ({current_user.get('role_name')})")
    else:
...
def render_file_list(storage_manager: StorageManager, is_admin: bool = False):
    ...
    with col1:
        category_filter = st.selectbox(
            ...
            options=["Tutte"] + list(storage_manager.categories.keys()),
