-- Script CORRETTO per creare le tabelle storage
-- Gestisce correttamente i tipi UUID e INTEGER
...
-- 9. Crea policy per download (tutti possono inserire i propri download) - CORRETTO
CREATE POLICY "storage_downloads_insert_policy" ON storage_downloads
    FOR INSERT WITH CHECK (downloaded_by = auth.uid());

-- 10. Crea policy per visualizzazione download (admin vede tutto, utenti solo i propri) - CORRETTO
CREATE POLICY "storage_downloads_select_policy" ON storage_downloads
    FOR SELECT USING (
        downloaded_by = auth.uid() OR
        EXISTS (
            SELECT 1 FROM users u 
            WHERE u.id = downloaded_by::text::integer  -- Converti UUID a INTEGER per confronto
            AND u.is_admin = true
        )
    );
