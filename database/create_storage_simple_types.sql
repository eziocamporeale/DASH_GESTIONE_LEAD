...
-- Usa solo INTEGER per evitare problemi di conversione UUID
...
-- 2. Crea la tabella per i download (usa INTEGER per downloaded_by)
CREATE TABLE IF NOT EXISTS storage_downloads (
    id SERIAL PRIMARY KEY,
    file_id INTEGER NOT NULL REFERENCES storage_files(id) ON DELETE CASCADE,
    downloaded_by INTEGER NOT NULL,  -- Usa INTEGER invece di UUID
...
-- 9. Crea policy per download (tutti possono inserire i propri download) - SEMPLIFICATO
CREATE POLICY "storage_downloads_insert_policy" ON storage_downloads
    FOR INSERT WITH CHECK (true);  -- Permetti a tutti di inserire download

-- 10. Crea policy per visualizzazione download (tutti possono vedere tutti i download) - SEMPLIFICATO
CREATE POLICY "storage_downloads_select_policy" ON storage_downloads
    FOR SELECT USING (true);  -- Permetti a tutti di vedere i download
